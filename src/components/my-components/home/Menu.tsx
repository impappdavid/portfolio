...
import { Sun, Moon, SquareTerminal } from "lucide-react";
...
const DIRECTORIES: Record<string, DirectoryData> = {
  ...
  experience: {
    name: "experience",
    description:
      "Professional background and internship history (6 months total).",
    targets: {},
    info: [
...
export default function MainMenu() {
  ...
  const currentDir = activeFolder ? `${BASE_PATH}\\${activeFolder}` : BASE_PATH;
  ...
  return (
    <div className="w-full max-h-[855px] p-4 flex flex-col font-mono text-zinc-700 dark:text-zinc-300 overflow-hidden select-none">
      {/* Top Bar / Header */}
      <div className="hidden lg:flex items-center justify-between pb-4 mb-4 border-b border-zinc-300 dark:border-zinc-900/80 shrink-0">
      ...
      {isTerminal ? (
        ...
        >
          <div className="flex flex-col gap-2">
            {isTyping ? (
              <div className="text-zinc-600 dark:text-zinc-400 pl-2 border-l border-zinc-300 dark:border-zinc-800 whitespace-pre-wrap leading-relaxed">
                {displayedBanner}
                <span className="inline-block w-2 h-3 bg-cyan-600 dark:bg-cyan-400 animate-pulse ml-0.5 align-middle" />
              </div>
            ) : (
              history.map((item, index) => (
                ...
                >
                  ...
                  {item.type === "output" && (
                    <div className="text-zinc-600 dark:text-zinc-400 pl-2 border-l border-zinc-300 dark:border-zinc-800">
                      {item.text}
                    </div>
                  )}

                  {item.type === "error" && (
                    <div className="text-red-600 dark:text-red-400/90 pl-2 border-l border-red-300 dark:border-red-900/50">
                      {item.text}
                    </div>
                  )}
                </div>
            ...
            {!isTyping && (
              ...
              >
                <span className="text-[#16a34a] dark:text-[#22C55E] shrink-0">
                  {currentDir} $&gt;
                </span>
                ...
                <input
                  ...
                  className="w-full bg-transparent outline-none text-zinc-800 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-700"
        ...
        >
          <motion.div
            ...
            className="text-zinc-600 dark:text-zinc-400 font-semibold text-base"
          >
            .menu
          </motion.div>
          ...
          <div className="flex flex-col gap-1.5 pl-2">
            <motion.div
              variants={menuItemVariants}
              className="text-zinc-900 dark:text-zinc-300 font-medium text-sm"
            >
              # About
            </motion.div>
            ...
            <motion.a
              variants={menuItemVariants}
              href="/about"
              className="flex items-center gap-2 text-xs w-fit text-zinc-600 dark:text-zinc-500 hover:text-[#16a34a] dark:hover:text-[#22C55E] transition-colors pl-2"
            >
              <span>&gt;</span>
          ...
          <div className="flex flex-col gap-1.5 pl-2">
            <motion.div
              variants={menuItemVariants}
              className="text-zinc-900 dark:text-zinc-300 font-medium text-sm flex gap-2 items-center"
            >
              # Experience
              <div className="text-xs text-zinc-500 dark:text-zinc-400">(6 months)</div>
            </motion.div>

            <motion.div
              variants={menuItemVariants}
              className="flex items-center gap-2 hover:text-[#16a34a] dark:hover:text-[#22C55E] transition-colors text-xs text-zinc-600 dark:text-zinc-500 pl-2"
            >
              <span>#</span>
              <span>
                Freelance Frontend Developer (Fiverr) | May 2025 – Aug 2025
            ...
            <motion.div
              variants={menuItemVariants}
              className="flex items-center gap-2 text-xs hover:text-[#16a34a] dark:hover:text-[#22C55E] transition-colors text-zinc-600 dark:text-zinc-500 pl-2"
            >
              <span>#</span>
              <span>
                Software Developer Intern (Webváltó Kft.) | Jan 2023 – Apr 2023
